...
export const useCompetitionVeganRewards = (userVeganReward: ReactText) => {
  const veganAsBigNumber = new BigNumber(userVeganReward as string)
  const veganBalance = getBalanceNumber(veganAsBigNumber)
  const veganPriceBusd = usePriceVeganBusd()
  return { veganReward: veganBalance, dollarValueOfVeganReward: veganBalance * veganPriceBusd.toNumber() }
}
